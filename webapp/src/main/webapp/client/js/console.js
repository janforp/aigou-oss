$(function() {
	...
	$(".showUserHistory").bind("click", function() {
		...
		var me = popoverObj[0];
		$(".user-history-popovered").each(function() {
					if (this !== me) {
						var $th = $(this);
						$th.removeClass("user-history-popovered")
								.popover("destroy");
					}
				})
		if (popoverObj.hasClass("user-history-popovered")) {
			popoverObj.removeClass("user-history-popovered").popover("destroy");
			...
			// onclick="$(\'.user-history-popovered-'
			// + dataId
			// + '\').popover(\'hide\')">×</button>&nbsp;');
			// }
			return;
		...
		util.ajax("/a/history/user-find", "userId=" + userId, function(json) {
			$this.removeClass("clicking");
			...
			if (json.success) {
				...
				content.push('</div>');
				var popoverId = "user-history-popovered-" + dataId;
				...
				popoverObj.addClass("user-history-popovered");
	...
	$(".showUserInfo").bind("click", function() {
		...
		var popoverObj = $this;
		$(".user-popovered").each(function() {
					if (this !== me) {
						var $th = $(this);
						$th.removeClass("user-popovered").popover("destroy");
					}
				})
		if ($this.hasClass("user-popovered")) {
			popoverObj.removeClass("user-popovered").popover("destroy");
		...
		popoverObj.popover(op).popover('show');
		util.ajax("/a/user/info", "userId=" + userId, function(json) {
			$this.removeClass("clicking");
			popoverObj.popover('destroy');
			if (json.success) {
				var content = [];
				var bean = json.bean;
				content.push('<div class="user-box show-form" role="form">');
				content.push('<div class="user-info">');
				content.push('<div class="user-portrait tc pb10">');
				content.push('<img class="border-radius-3" width="80" src="');
				content.push(bean.userHeaderPortrait);
				content.push('">');
				content.push('</div>');
				content.push('<dl class="dl-horizontal">');
				content.push('<dt class="user-label">');
				...
				if (bean.blockStatus == null || bean.blockStatus < 2) {
					...
					blockStatusClass = "user-reportStatus-" + bean.blockStatus;
				}
				...
				/*
				 * content.push('<dl class="dl-horizontal">'); content.push('<dt class="user-label">');
				...
				content.push('<label class="user-label">');
				...
				content.push('</div>');
				var popoverId = "user-info-popovered-" + userIdForPopover;
				var popoverId4Btn = "btn-" + popoverId;
				var op2 = {
	...
	$showUserDetail.bind("click", function() {
				...
				var $this = $(this);
				var userId = $this.attr("data-user-id");
				util.dialog.open("/a/user/detail?userId=" + userId, {
							showHeader : false
						});
			});
